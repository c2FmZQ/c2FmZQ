...
ADD kringle/go.mod /app/go/src/kringle/go.mod
ADD kringle/go.sum /app/go/src/kringle/go.sum
WORKDIR /app/go/src/kringle
RUN go mod download

ADD kringle /app/go/src/kringle
RUN go install ./kringle-server
